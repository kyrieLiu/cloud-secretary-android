 ...
 * @Describe 自定义tabLayout,可自定义下划线长度,角标
 */
@ViewPager.DecorView
class MarkTabLayout(ctx: Context, attrs: AttributeSet) : HorizontalScrollView(ctx, attrs) {

    private val tabs = mutableListOf<MarkTab>()
    private var selectedTab: MarkTab? = null
    ...
    private var tabIconTintMode: android.graphics.PorterDuff.Mode?
    ...
    private val tabViewPool = Pools.SimplePool<MarkTabView>(12)
    ...
    init {
        ...
        val a = ctx.obtainStyledAttributes(attrs, R.styleable.MarkTabLayout, R.attr.MarkTabStyle, R.style.MarkTabLayoutStyle)

        slidingTabIndicator.setSelectedIndicatorHeight(a.getDimensionPixelSize(R.styleable.MarkTabLayout_fTabIndicatorHeight, -1))
        slidingTabIndicator.setSelectedIndicatorColor(a.getColor(R.styleable.MarkTabLayout_fTabIndicatorColor, 0))
        tabSelectedIndicator = a.getDrawable(R.styleable.MarkTabLayout_fTabIndicator)
        tabIndicatorGravity = a.getInt(R.styleable.MarkTabLayout_fTabIndicatorGravity, INDICATOR_GRAVITY_BOTTOM)
        tabIndicatorFullWidth = a.getBoolean(R.styleable.MarkTabLayout_fTabIndicatorFullWidth, true)
        tabIndicatorFixedWidth = a.getDimensionPixelSize(R.styleable.MarkTabLayout_fTabIndicatorFixedWidth, 0)

        tabPaddingStart = a.getDimensionPixelSize(R.styleable.MarkTabLayout_fTabPadding, 0)
        tabPaddingTop = tabPaddingStart
        tabPaddingEnd = tabPaddingStart
        tabPaddingBottom = tabPaddingStart
        tabPaddingStart = a.getDimensionPixelSize(R.styleable.MarkTabLayout_fTabPaddingStart, tabPaddingStart)
        tabPaddingTop = a.getDimensionPixelSize(R.styleable.MarkTabLayout_fTabPaddingTop, tabPaddingTop)
        tabPaddingEnd = a.getDimensionPixelSize(R.styleable.MarkTabLayout_fTabPaddingEnd, tabPaddingEnd)
        tabPaddingBottom = a.getDimensionPixelSize(R.styleable.MarkTabLayout_fTabPaddingBottom, tabPaddingBottom)
        ...
        if (a.hasValue(R.styleable.MarkTabLayout_fTabTextColor)) {
            tabTextColors = getColorStateList(context, a, R.styleable.MarkTabLayout_fTabTextColor)
        }

        if (a.hasValue(R.styleable.MarkTabLayout_fTabSelectedTextColor)) {
            val selected = a.getColor(R.styleable.MarkTabLayout_fTabSelectedTextColor, 0)
            tabTextColors = createColorStateList(tabTextColors!!.defaultColor, selected)
        }

        tabIconTint = getColorStateList(context, a, R.styleable.MarkTabLayout_fTabIconTint)
        tabIconTintMode = parseTintMode(a.getInt(R.styleable.MarkTabLayout_fTabIconTintMode, -1), null)

        tabRippleColorStateList = getColorStateList(context, a, R.styleable.MarkTabLayout_fTabRippleColor)

        tabIndicatorAnimationDuration = a.getInt(R.styleable.MarkTabLayout_fTabIndicatorAnimationDuration, ANIMATION_DURATION)

        requestedTabMinWidth = a.getDimensionPixelSize(R.styleable.MarkTabLayout_fTabMinWidth, INVALID_WIDTH)
        requestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.MarkTabLayout_fTabMaxWidth, INVALID_WIDTH)
        tabBackgroundResId = a.getResourceId(R.styleable.MarkTabLayout_fTabBackground, 0)
        contentInsetStart = a.getDimensionPixelSize(R.styleable.MarkTabLayout_fTabContentStart, 0)

        mode = a.getInt(R.styleable.MarkTabLayout_fTabMode, MODE_FIXED)
        tabGravity = a.getInt(R.styleable.MarkTabLayout_fTabGravity, GRAVITY_FILL)
        inlineLabel = a.getBoolean(R.styleable.MarkTabLayout_fTabInlineLabel, false)
        unboundedRipple = a.getBoolean(R.styleable.MarkTabLayout_fTabUnboundedRipple, false)
        a.recycle()


        tabTextMultiLineSize = resources.getDimensionPixelSize(R.dimen.sp_12).toFloat()
        scrollableTabMinWidth = resources.getDimensionPixelSize(R.dimen.dp_72)
    ...
    fun addTab(tab: MarkTab) {
        addTab(tab, tabs.isEmpty())
    }

    fun addTab(tab: MarkTab, position: Int) {
        addTab(tab, position, tabs.isEmpty())
    }

    fun addTab(tab: MarkTab, setSelected: Boolean) {
        addTab(tab, tabs.size, setSelected)
    }

    fun addTab(tab: MarkTab, position: Int, setSelected: Boolean) {
        if (tab.parent != this) {
    ...
    private fun addTabFromItemView(item: MarkTabItem) {
        val tab = newTab()
        if (item.text != null) {
            tab.text = item.text
        }
    ...
    protected fun createTabFromPool(): MarkTab {
    ...
    protected fun releaseFromTabPool(tab: MarkTab): Boolean {
        return tabPool.release(tab)
    }

    fun getTabCount(): Int = tabs.size

    fun getTabAt(index: Int): MarkTab? = if (index < 0 || index >= getTabCount()) null else tabs[index]

    fun getSelectedTabPosition(): Int = selectedTab?.position ?: -1

    fun removeTab(tab: MarkTab) {
        if (tab.parent != this) {
    ...
    fun addDotBadge(index: Int): DotBadge =
            DotBadge(Color.RED, dpToPx(4)).apply {
                addBadge(index, this)
            }

    fun addNumberBadge(index: Int, number: Int, color: Int = Color.parseColor("#EF6732"), textColor: Int = Color.WHITE, textSize: Int = dpToPx(DEFAULT_NUMBER_BADGE_TEXT_SIZE)) =
    ...
    fun addBadge(index: Int, badge: Badge) {
        (slidingTabIndicator.getChildAt(index) as? MarkTabView)?.badge = badge
    }

    fun removeBadge(index: Int) {
        (slidingTabIndicator.getChildAt(index) as? MarkTabView)?.badge = null
    }

    fun getBadge(index: Int): Badge? = (slidingTabIndicator.getChildAt(index) as? MarkTabView)?.badge

    fun setupWithViewPager(viewPager: ViewPager?) {
        setupWithViewPager(viewPager, true)
    }

    private fun setupWithViewPager(viewPager: ViewPager?, autoRefresh: Boolean, implicitSetup: Boolean = false) {
    ...
    private fun createTabView(tab: MarkTab): MarkTabView {
        var tabView = tabViewPool.acquire()
        if (tabView == null) {
            tabView = MarkTabView(context)
        }
        tabView.tab = tab
    ...
    private fun configureTab(tab: MarkTab, position: Int) {
    ...
    private fun addTabView(tab: MarkTab) {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (childCount == 1) {
            ...
            val remeasure = when (mode) {
                MODE_SCROLLABLE -> child.measuredWidth < measuredWidth
    ...
    private fun removeTabViewAt(position: Int) {
        val view = slidingTabIndicator.getChildAt(position) as? MarkTabView
    ...
    private fun animateToTab(newPosition: Int) {
        if (newPosition == MarkTab.INVALID_POSITION) {
            return
        }
    ...
    internal fun selectTab(tab: MarkTab?, updateIndicator: Boolean = true) {
        ...
        if (currentTab == tab) {
            ...
            val newPosition = tab?.position ?: MarkTab.INVALID_POSITION
            if (updateIndicator) {
                if ((currentTab == null || currentTab.position == TabLayout.Tab.INVALID_POSITION) && newPosition != MarkTab.INVALID_POSITION) {
                    setScrollPosition(newPosition, 0f, true)
                } else {
    ...
    private fun dispatchTabSelected(tab: MarkTab) {
        selectedListeners.forEach { it.onTabSelected(tab) }
    }

    private fun dispatchTabUnselected(tab: MarkTab) {
        selectedListeners.forEach { it.onTabUnselected(tab) }
    }

    private fun dispatchTabReselected(tab: MarkTab) {
    ...
    private fun applyModeAndGravity() {
        val paddingStart = if (mode == MODE_SCROLLABLE) Math.max(0, contentInsetStart - tabPaddingStart) else 0
    ...
    inner class MarkTabView(ctx: Context) : LinearLayout(ctx) {

        var tab: MarkTab? = null
            set(value) {
                field = value
                update()
            }
        ...
        override fun onMeasure(origWidthMeasureSpec: Int, origHeightMeasureSpec: Int) {
            ...
            val specWidthMode = View.MeasureSpec.getMode(origHeightMeasureSpec)
            val maxWidth = tabMaxWidth

            val widthMeasureSpec = if (maxWidth > 0 && (specWidthMode == View.MeasureSpec.UNSPECIFIED || specWidthSize > maxWidth)) {
        ...
        fun update() {
            if (iv == null) {
                iv = LayoutInflater.from(context).inflate(R.layout.layout_mark_tab_icon, this, false) as ImageView
                addView(iv, 0)
            }
            if (tv == null) {
                tv = LayoutInflater.from(context).inflate(R.layout.layout_mark_tab_text, this, false) as TextView
                addView(tv)
    ...
    private inner class SlidingTabIndicator internal constructor(context: Context) : LinearLayout(context) {
        ...
        override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
            ...
            if (mode == MODE_FIXED && tabGravity == GRAVITY_CENTER) {
                ...
                if (largestTabWidth * count <= measuredWidth - gutter * 2) for (i in 0 until count) {
        ...
        private fun updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.width > 0) {
                ...
                if (selectedTitle is MarkTabView) {
            ...
            if (selectedPosition < childCount - 1) {
                (selectedTitle as MarkTabView).updateTextColor(getTextColorByFraction(1 - selectionOffset))
                (getChildAt(selectedPosition + 1) as MarkTabView).updateTextColor(getTextColorByFraction(selectionOffset))
            }
            ...
            if (startLeft != finalTargetLeft || startRight != finalTargetRight) {
                indicatorAnimator = ValueAnimator().apply {
                    ...
                    addUpdateListener {
                        ...
                        (getChildAt(position) as MarkTabView).updateTextColor(getTextColorByFraction(animatedFraction))
                        (getChildAt(selectedPosition) as MarkTabView).updateTextColor(getTextColorByFraction(1 - animatedFraction))
                    }
        ...
        override fun draw(canvas: Canvas) {
            ...
            if (indicatorLeft in 0..(indicatorRight - 1)) {
    ...
    class TabLayoutOnPageChangeListener(tabLayout: MarkTabLayout) : ViewPager.OnPageChangeListener {
        private val tabLayoutRef = WeakReference<MarkTabLayout>(tabLayout)
    ...
    class ViewPagerOnTabSelectedListener(private val viewPager: ViewPager) : OnTabSelectedListener {
        ...
        override fun onTabReselected(tab: MarkTab) {}

        override fun onTabSelected(tab: MarkTab) {
            viewPager.currentItem = tab.position
        }

        override fun onTabUnselected(tab: MarkTab) {}
    }
    ...
    private inner class AdapterChangeListener(var autoRefresh: Boolean = false) : ViewPager.OnAdapterChangeListener {
        ...
        override fun onAdapterChanged(viewPager: ViewPager, oldAdapter: PagerAdapter?, newAdapter: PagerAdapter?) {
            if (this@MarkTabLayout.viewPager == viewPager) {
    ...
    companion object {
        ...
        private val tabPool = Pools.SynchronizedPool<MarkTab>(16)
    }
